...
*   **附注 (本地/Cloudflare 测试):** 尽管使用 `@neondatabase/serverless` 将代码重构为 Edge Runtime 已成功，但通过 `npm run dev` 进行本地测试时遇到了 `fetch failed` 错误（这可能是本地模拟环境限制）。**后续在 Cloudflare Pages 部署环境中也遇到了同样的 `fetch failed` 错误，最终确定根本原因是 Cloudflare 环境变量 `DATABASE_URL` 配置错误。用户修正该变量后，部署环境中的问题已解决。** 功能已在部署环境中验证。
